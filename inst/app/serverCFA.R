server_cfa <- function(input, output, session) {
  ...
  output$fit_comparison <- renderUI({
    ...
    column(12,
           ...
           #     tags$thead(
           #       tags$tr(
           #         tags$th("Model", style = "min-width: 100px;"),
           #         tags$th("χ²", style = "text-align: center;"),
           #         tags$th("df", style = "text-align: center;"),
           #         tags$th("p-value", style = "text-align: center;"),
           #         tags$th("RMSEA", style = "text-align: center;"),
           #         tags$th("CFI", style = "text-align: center;"),
           #         tags$th("TLI", style = "text-align: center;"),
           #         tags$th("GFI", style = "text-align: center;"),
           #         tags$th("SRMR", style = "text-align: center;"),
           #         tags$th("NFI", style = "text-align: center;"),
           ...
           #         status_info <- get_fit_status(m["chisq"],m["df"],m["pvalue"], m["rmsea"], m["cfi"], m["srmr"], m["gfi"])
           #         
           #         tags$tr(
           #           style = if(i == length(lst)) "background-color: #e8f5e8; font-weight: bold;" else "",
           #           tags$td(
           #             if(i == length(lst)) tags$span(x$id, style = "color: #2c3e50;") else x$id,
           #             style = "font-weight: bold;"
           #           ),
           #           tags$td(round(m["chisq"], 2), 
           #                   style = paste("text-align: center; color:", 
           #                                 ifelse(is.na(m["chisq"]), "black",
           #                                        ifelse(m["chisq"]/m["df"] < 2, "green",
           #                                               ifelse(m["chisq"]/m["df"] < 5, "orange", "red"))))
           #           ),
           #           tags$td(m["df"], 
           #                   style = paste("text-align: center; color:", 
           #                                 ifelse(is.na(m["chisq"]), "black",
           ...
           #           ),
           #           tags$td(
           #             ifelse(is.na(m["pvalue"]), "NA", 
           #                    ifelse(m["pvalue"] < 0.001, "< .001", sprintf("%.3f", m["pvalue"]))),
           #             style = paste("text-align: center; color:", 
           #                           ifelse(is.na(m["pvalue"]), "black",
           #                                  ifelse(m["pvalue"] >= 0.1, "green",
           #                                         ifelse(m["pvalue"]>= 0.05, "green", "red"))))
           #           ),
           #           tags$td(
           #             ifelse(is.na(m["rmsea"]), "NA", sprintf("%.3f", m["rmsea"])),
           #             style = paste("text-align: center; color:", 
           #                           ifelse(is.na(m["rmsea"]), "black",
           #                                  ifelse(m["rmsea"] < 0.08, "green",
           #                                         ifelse(m["rmsea"] < 0.1, "orange", "red"))))
           #           ),
           #           tags$td(
           #             ifelse(is.na(m["cfi"]), "NA", sprintf("%.3f", m["cfi"])),
           #             style = paste("text-align: center; color:", 
           #                           ifelse(is.na(m["cfi"]), "black",
           #                                  ifelse(m["cfi"] >= 0.9, "green",
           #                                         ifelse(m["cfi"] >= 0.88, "orange", "red"))))
           #           ),
           #           tags$td(
           #             ifelse(is.na(m["tli"]), "NA", sprintf("%.3f", m["tli"])),
           #             style = paste("text-align: center; color:", 
           #                           ifelse(is.na(m["tli"]), "black",
           #                                  ifelse(m["tli"] >= 0.9, "green",
           #                                         ifelse(m["tli"] >= 0.88, "orange", "red"))))
           #           ),
           #           tags$td(
           #             ifelse(is.na(m["gfi"]), "NA", sprintf("%.3f", m["gfi"])),
           #             style = paste("text-align: center; color:", 
           #                           ifelse(is.na(m["gfi"]), "black",
           #                                  ifelse(m["gfi"] >= 0.9, "green",
           #                                         ifelse(m["gfi"] >= 0.88, "orange", "red"))))
           #           ),
           #           tags$td(
           #             ifelse(is.na(m["srmr"]), "NA", sprintf("%.3f", m["srmr"])),
           #             style = paste("text-align: center; color:", 
           #                           ifelse(is.na(m["srmr"]), "black",
           #                                  ifelse(m["srmr"] <= 0.05, "green",
           #                                         ifelse(m["srmr"] <= 0.08, "orange", "red"))))
           #           ),
           #           tags$td(
           #             ifelse(is.na(m["nfi"]), "NA", sprintf("%.3f", m["nfi"])),
           #             style = paste("text-align: center; color:", 
           #                           ifelse(is.na(m["nfi"]), "black",
           #                                  ifelse(m["nfi"] >= 0.9, "green",
           #                                         ifelse(m["nfi"] >= 0.88, "orange", "red"))))
           #           ),
           #           
           #           # KOLOM STATUS 
           #           tags$td(
           #             status_info$status,
           #             style = paste("text-align: center; font-weight: bold; background-color:", 
           #                           status_info$color)
           #           )
           #           
           #         )
           ...
           #     )
           #   )),
           # --- Tetap: Ambil measures dan hitung status ---
           
           
           # --- Diganti: render table HTML manual → DT =====
